\ctable[caption={Studying a problem at multiple scales},
		...
		square in four sub blocks, where you repeat the same process for each
		 of the sub blocks until you reach unity.}}{\FL
...
\ctable[caption={An illustration of how the Renormalization algorithm works},
	...
	figure]{c}{\tnote[]{
	\begin{enumerate}
	\item Determine the optimal route in a basic two by two cell. The dashed
	lines are the edges which can be traversed. The open circles are border
	points and the closed circles are cell points.
	\item Based on the starting and end border node in a block, lookup the
	optimal path visiting the cells where at least one city is located.\\
	\item Divide the block into four sub blocks, use the crossings of the route
	with the borders of a cell (The X) as new starting and end nodes.\\
	\item These sub blocks are treated as a block again, and the process
	restarts from picture B. This dividing continues until at most one city is
	in each cell.
	\end{enumerate}}}{\FL
...
When the first iteration is finished we can use earlier blocks to calculate an
